...
type CanonicalConfig struct {
	...
	PeriodicUpdateInterval int

	DetectExternalChanges bool

	SessionRefreshInterval int

	MaxSessionRefreshInterval int
...
const (
	...
	configKeyPeriodicUpdateInterval    = "periodic_update_interval"
	configKeyDetectExternalChanges     = "detect_external_changes"
...
func NewConfig(logger *zap.SugaredLogger, notifier Notifier) (*CanonicalConfig, error) {
	...
	userConfig.SetDefault(configKeyPeriodicUpdateInterval, 10)
	userConfig.SetDefault(configKeyDetectExternalChanges, true)
...
func (cc *CanonicalConfig) populateFromVipers() error {
	...
	cc.logger.Debugw("Loaded session refresh config",
		...
		"MaxSessionRefreshInterval", cc.MaxSessionRefreshInterval,
		"FastSessionDetection", cc.FastSessionDetection)
